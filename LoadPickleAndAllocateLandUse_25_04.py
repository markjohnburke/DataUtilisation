...
###CODE HERE TO ALLOCATE TO AIRBNB DATASET THE ASSIGNED "LAND USE CODE" AS APPROPRIATE
...
df_airbnb.to_pickle('./airbnbAndLandUseUpdated.pkl')
...
df_cdrs_internet = pd.read_pickle("./completedInternet14DaysSattoSat.pkl")
##CODE FOR ADDING WHOLE CONNECTIVITY spectrum - 14 days - to ydata
propArray = []
airCell = 1
ydata = pd.Series([])
for i in range(1,len(df_airbnb)): 
    airCell = df_airbnb.cellID[i]
    ydata = df_cdrs_internet[df_cdrs_internet.CellID==airCell]['internet']
    propArray.append(ydata)
    #df_airbnb['neighConnection'][i] = {'neighConnection': [ydata]}
    #print(df_airbnb)
